...
class GroupLogger:
    ...
    def mark_deleted_all_chats(self, msg_ids: list[int]):
        """Пометить удалённые сообщения во всех файлах (когда chat_id неизвестен)."""
        try:
            for p in self.dir.glob("group_*.json"):
                try:
                    data = json.loads(p.read_text("utf-8"))
                    changed = False
                    for mid in msg_ids:
                    ...
                    if changed:
                        p.write_text(json.dumps(data, ensure_ascii=False, indent=2), "utf-8")
                        logger.info(f"mark_deleted_all_chats: помечено в {p.name}")
                except Exception:
                    continue
        except Exception as e:
            logger.warning(f"mark_deleted_all_chats: {e}")
...
class JarvisAgent:
    ...
    async def web_search(self, query: str, n: int = 5) -> list[str]:
        ...
        for backend in ("html", "lite", "api"):
            try:
                ...
                with DDGS(headers={"User-Agent": _ua}, timeout=15) as ddgs:
                ...
                err = str(e)
                if any(w in err.lower() for w in ("network","timeout","connect","ratelimit","blocked","403","202")):
                    continue
    ...
    async def system_check(self) -> str:
        ...
        if config.TAVILY_API_KEY:
            ...
            results.append(("Tavily (основной поиск)", False,
                "⚠️ не настроен — без Tavily поиск хуже. Добавь TAVILY_API_KEY в .env"))

        # ── 5. httpx — чтение страниц ─────────────────────────
        try:
            async with httpx.AsyncClient(
        ...
        # ── 10. Напоминания ───────────────────────────────────
        try:
            total_rem = len([r for r in self.reminders._data if not r.get("done")])
        ...
        try:
            profiles_count = len(list(Path("user_profiles").glob("*.json"))) if Path("user_profiles").exists() else 0
        ...
        results.append(("История чатов", True, f"файлов: {len(history_files)}"))
