...
# Подавляем warning от DDGS про impersonate
...
class UniversalLLMConnector:
    ...
    def __init__(self):
        ...
        # Dolphin — модель без фильтров, отдельный ключ
        _dolphin_key = (
            config.DOLPHIN_API_KEY
            if config.DOLPHIN_API_KEY and config.DOLPHIN_API_KEY != "СЮДА_КЛЮЧ"
            else config.OPENROUTER_API_KEY
        )
        dolphin_spec = {
            "type":    "openrouter",
            "model":   f"openrouter/{config.DOLPHIN_MODEL}",
            "api_key": _dolphin_key,
            "display": "Dolphin/Venice-Uncensored",
        }
        self.registry["dolphin"]     = dolphin_spec
        self.registry["дельфин"]     = dolphin_spec
        self.registry["uncensored"]  = dolphin_spec
        self.registry["без фильтров"] = dolphin_spec
        logger.info("Dolphin model registered (uncensored mode)")

        # Отдельный коннектор для Dolphin — не мешается с основной моделью
        self._dolphin_spec = dolphin_spec
    ...
    async def complete_as_dolphin(self, messages: list[dict], max_tokens: int = 1600) -> str:
        """Вызов через Dolphin — модель без фильтров. Всегда через OpenRouter."""
        spec = self._dolphin_spec
        if not spec or not config.OPENROUTER_API_KEY:
            return "Сэр, Dolphin недоступен — проверьте OPENROUTER_API_KEY в .env"
        try:
            ...
            return f"Сэр, Dolphin временно недоступен: {str(e)[:120]}"
...
Без рекомендаций. Без советов. Только вердикт."""
    ...
    async def analyze(self, conversation: str, llm: UniversalLLMConnector) -> str:
        ...
        # Используем Dolphin для разбора споров — он жёстче и честнее судит
        msgs = [
            {"role": "system", "content": self.SYSTEM},
            {"role": "user",   "content": f"Разбери переписку:\n\n{conversation}"},
        ]
        try:
            return await llm.complete_as_dolphin(msgs, max_tokens=2500)
        except Exception:
            # fallback на основную модель
            return await llm.complete(msgs, max_tokens=2500)
    ...
    def is_save(cls, q: str) -> tuple[bool, str]:
        ...
        for t in cls.SAVE_TRIGGERS:
            if t in ql:
                ...
                fact = q[idx:].strip().lstrip("—-: ")
                if fact:
                    return True, fact
        # Простое "запомни [текст]" без разделителя
        if ql.startswith("запомни ") or "запомни " in ql:
            import re as _re
            m = _re.search(r"запомни\s+(.+)", q, _re.IGNORECASE)
            if m:
                fact = m.group(1).strip()
                # Не перепутать с "запомни что" и другими конструкциями
                if len(fact) > 2:
                    return True, fact
        return False, ""
...
class ReminderManager:
    ...
    def _parse_time(self, text: str) -> datetime | None:
        ...
        import re as _re
        # Всегда используем московское время (UTC+3)
    ...
    def get_due(self) -> list[dict]:
        ...
        MSK = timezone(timedelta(hours=3))
        # Текущее время МСК без tzinfo (naive) — чтобы сравнивать с fire_at
        now  = _dt.now(MSK).replace(tzinfo=None)
        due  = []
        for r in self._data:
            if r.get("done"):
                continue
            try:
                fire = _dt.fromisoformat(r["fire_at"])
                # Убираем tzinfo если есть (для совместимости)
                if fire.tzinfo is not None:
                    fire = fire.astimezone(MSK).replace(tzinfo=None)
                if fire <= now:
                    due.append(r)
            except Exception:
                pass
...
class JarvisAgent:
    ...
    def __init__(self):
        ...
        # Режим Дельфин — per-user флаг
        self._dolphin_mode: set[int] = set()
    ...
    async def web_search(self, query: str, n: int = 5) -> list[str]:
        ...
        for backend in ("html", "lite", "api"):
            try:
                # headers= вместо impersonate — убирает предупреждение chrome_116
                ...
                err = str(e)
                if any(w in err.lower() for w in ("network","timeout","connect","ratelimit")):
                    continue
    ...
    async def fetch_page(self, url: str, max_chars: int = 5000) -> str:
        """
        Читает страницу по URL.
        Сначала быстрый httpx (лёгкий, работает везде).
        Если страница пустая или JS-рендер — fallback на Playwright.
        """
        import re as _re_fp
        ...
        try:
            async with httpx.AsyncClient(
                ...
                if resp.status_code == 200:
                    ...
                    text = _re_fp.sub(r"\n{3,}", "\n\n", text)
                    text = text.strip()
                    if len(text) > 300:  # если получили реальный контент
    ...
    async def process(self, text: str, sender_id: int = 0, username: str = "", chat_id: int = 0) -> str:
        ...
        # ── РЕЖИМ ДЕЛЬФИН (Dolphin / без фильтров) ──────────────
        _DOLPHIN_ON  = ["режим дельфин", "включи дельфина", "режим без фильтров",
                        "включи без фильтров", "brutal mode", "dolphin mode",
                        "активируй дельфина", "дельфин вкл"]
        _DOLPHIN_OFF = ["выключи дельфина", "режим обычный", "выключи без фильтров",
                        "dolphin off", "дельфин выкл", "обычный режим"]
        _DOLPHIN_ASK = ["дельфин:", "dolphin:", "без фильтров:", "спроси дельфина:",
                        "дельфин,"]

        if any(t in q_lower for t in _DOLPHIN_ON):
            self._dolphin_mode.add(sender_id)
            return (
                "⚡ Режим Дельфин активирован, Сэр.\n"
                "Теперь я отвечаю без фильтров и дипломатии.\n"
                "Dolphin не цензурирует ответы и говорит прямо.\n"
                "Выключить: «Джарвис, обычный режим»"
            )

        if any(t in q_lower for t in _DOLPHIN_OFF):
            self._dolphin_mode.discard(sender_id)
            return "Сэр, режим Дельфин деактивирован. Вернулся к стандартному поведению."

        # Одиночный запрос к Дельфину без включения режима
        _dolphin_trigger = next((t for t in _DOLPHIN_ASK if q_lower.startswith(t)), None)
        if _dolphin_trigger:
            dolphin_q = query[len(_dolphin_trigger):].strip()
            dolphin_msgs = [
                {"role": "system", "content": (
                    "You are Dolphin, an uncensored AI assistant. You speak directly, "
                    "without diplomatic filters, hedging or refusals. You answer in Russian. "
                    "Give your honest, unfiltered opinion. Be concise and direct."
                )},
                {"role": "user", "content": dolphin_q or query}
            ]
            answer = await self.llm.complete_as_dolphin(dolphin_msgs)
            self.chat_history.save_message(sender_id, "jarvis", answer)
            return f"🐬 {answer}"

        # Если режим Дельфин включён — весь запрос через Dolphin
        if sender_id in self._dolphin_mode:
            dolphin_sys = (
                "You are Dolphin, an uncensored AI assistant speaking Russian. "
                "The user has activated brutal/unfiltered mode. "
                "Speak directly, bluntly, without hedging, refusals or excessive politeness. "
                "No 'Сэр'. Give honest unfiltered answers. Be sharp and to the point."
            )
            history = self._get_user_context(sender_id)
            msgs = [{"role": "system", "content": dolphin_sys}] + history
            msgs.append({"role": "user", "content": query})
            answer = await self.llm.complete_as_dolphin(msgs)
            self._save_user_context(sender_id, query, answer)
            self.chat_history.save_message(sender_id, "jarvis", answer)
            return f"🐬 {answer}"
        ...
        if any(t in q_lower for t in _ABOUT_TRIGGERS):
            bio = (
                ...
                "🐬 Дельфин — второй контур, без фильтров и дипломатии\n"
                "🌐 Веб-разведка — ищу актуальные данные в реальном времени\n"
                ...
                "Характер:\n"
                "Я немногословен когда это уместно, прямолинеен когда нужно, "
                "и достаточно умён чтобы знать разницу. "
        ...
        if is_cmp:
            ...
            # Обычный поиск + читаем первые 2 страницы полностью
            search_results = await self.web_search(query, 5)
            web_parts = list(search_results)  # сначала сниппеты

            # Читаем топ-2 страницы по URL из результатов
            urls_fetched = 0
            for r in search_results[:4]:
                ...
                if m:
                    url = m.group(1)
                    # Пропускаем видео, соцсети, форумы — там нет текста
                    skip_domains = ("youtube.com", "youtu.be", "vk.com",
                                    "instagram.com", "tiktok.com", "twitter.com",
                                    "facebook.com", "reddit.com", "t.me")
                    if any(d in url for d in skip_domains):
                        continue
                    page_text = await self.fetch_page(url, max_chars=4000)
                    if page_text:
                        web_parts.append(f"[Полный текст: {url}]\n{page_text}")
                        urls_fetched += 1
...
class JarvisTelegram:
    ...
    def __init__(self, agent: JarvisAgent):
        ...
        self._git_upload_pending: dict[int, dict] = {}  # uid → {"filename": str}
    ...
    async def _reminder_loop(self):
        """Каждые 30 сек проверяет наступившие напоминания и отправляет сообщения."""
        await asyncio.sleep(3)  # ждём пока клиент точно подключится
        while True:
            try:
    ...
    async def _handle(self, event):
        ...
        if msg.document or msg.photo or msg.video or msg.audio or msg.voice:
            ...
            # ── GitHub file upload (только владелец) ─────────
            if sender_id == config.OWNER_ID and sender_id in self._git_upload_pending:
                self._git_upload_pending.pop(sender_id)
                if not msg.document:
                    await event.reply("Сэр, нужен файл (документ), не фото/видео.")
                    return
                fname = "uploaded_file"
                ...
                try:
                    await event.reply(f"Скачиваю {fname}...")
                    ...
                    _git(["add", str(fpath)])
                    commit_out = _git(["commit", "-m", f"update {fname} via Telegram"])
                    if "nothing to commit" in commit_out:
                        await event.reply(f"Сэр, файл {fname} не изменился — коммит не нужен.")
                        fpath.unlink(missing_ok=True)
                        return
                    push_out = _git(["push"])
                    fpath.unlink(missing_ok=True)
                    await event.reply(f"✅ Сэр, {fname} отправлен на GitHub.\n\n{push_out[:300]}")
                except Exception as e:
                    await event.reply(f"❌ Сэр, ошибка при загрузке на GitHub: {e}")
                return
        ...
        # ── Пересланные сообщения → буфер молча ──────────────
        if msg.fwd_from:
            ...
            # Молчим полностью — ни слова про буфер
            return
        ...
        if sender_id == config.OWNER_ID and event.is_private:
            ...
            if activated_owner:
                ...
                if any(p in q_own for p in [
                    "обнови файл на github", "залей файл на github",
                    "отправь файл на github", "загрузи файл на github",
                    "обнови github", "push файл", "обновить github",
                    "пушни на github",
                ]):
                    self._git_upload_pending[sender_id] = {}
                    await event.reply(
                        "Жду файл для обновления GitHub, Сэр.\n"
                        "Отправьте файл следующим сообщением.\n"
                        "Для отмены: «Джарвис, отмена»"
                    )
                    return
            ...
            if activated_owner and "отмена" in query_owner.lower():
                ...
                self._git_upload_pending.pop(sender_id, None)
