...
class JarvisAgent:
    ...
    async def system_check(self) -> str:
        ...
        # ── 6. Playwright — браузер для JS-сайтов (опционально) ──
        try:
            ...
            err_str = str(e)[:100]
            # Playwright не обязателен — помечаем как предупреждение, не ошибку
            results.append(("Playwright (браузер)", True,
                f"⚠️ не установлен на этом хостинге (опционально) — httpx справляется"))
        ...
        try:
            ...
            if not db_path.exists():
                results.append(("Ночной бэкап БД", False, "Jarvis.db не найдена — БД создастся автоматически"))
            else:
                test_backup = db_path.parent / "_backup_test.db"
                _sh.copy2(str(db_path), str(test_backup))
                _conn = _sq.connect(str(test_backup))
                _tables = _conn.execute("SELECT name FROM sqlite_master WHERE type='table'").fetchall()
                _table_names = [t[0] for t in _tables]
                _msgs = _conn.execute("SELECT COUNT(*) FROM messages").fetchone()[0] if "messages" in _table_names else 0
                _grps = _conn.execute("SELECT COUNT(*) FROM group_messages").fetchone()[0] if "group_messages" in _table_names else 0
                _conn.close()
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                ...
                _now = datetime.now(_tz(_td(hours=3))).replace(tzinfo=None)
        ...
        if is_owner and is_pm and any(t in text.lower() for t in _LOG_TRIGGERS):
            ...
            if log_path.exists() and log_path.stat().st_size > 0:
                try:
                    ...
                    from datetime import timezone as _ltz, timedelta as _ltd
                    _msk_now = datetime.now(_ltz(_ltd(hours=3))).replace(tzinfo=None)
...
class GroupMonitor:
    ...
    def __init__(self, agent: JarvisAgent):
        ...
        if config.USER_SESSION_STRING:
            ...
            logger.info("GroupMonitor: используется StringSession из .env")
        else:
...
async def main():
    ...
    try:
        ...
        if _db_path.exists() and _db_path.stat().st_size > 0:
            ...
            # Проверяем только наличие файла, таблицы могут ещё не быть
            _tables = _tc.execute(
            ...
            _test_bk.unlink()
            if _tables:
                logger.info(f"✅ Система бэкапа: функционирует — таблиц: {len(_tables)}, отправка в 03:00 МСК")
                print("✅ Система бэкапа: функционирует")
            else:
                logger.info("✅ Система бэкапа: копирование работает (БД пуста — таблицы создадутся при первом старте)")
                print("✅ Система бэкапа: копирование работает")
        else:
            logger.info("ℹ️  Система бэкапа: БД не найдена — создастся автоматически при первом сообщении")
            print("ℹ️  Система бэкапа: ждёт создания БД")
