async def _restore_from_gdrive_if_empty():
    """При пустой БД уведомляет владельца и ждёт .db файл в Telegram."""
    try:
        ...
        if not config.OWNER_ID:
            return
        logger.info("БД пустая — уведомляем владельца")
        # Уведомление отправится после подключения Telegram — сохраняем флаг
        _jarvis_db._q(
            "CREATE TABLE IF NOT EXISTS _startup_flags (key TEXT PRIMARY KEY, val TEXT)"
        )
        _jarvis_db._q(
            "INSERT OR REPLACE INTO _startup_flags (key, val) VALUES ('db_empty_notify', '1')"
        )
    except Exception as e:
        logger.warning(f"⚠️ Проверка БД: {e}")
...
class JarvisTelegram:
        ...
        try:
            ...
            if flag and config.OWNER_ID:
                ...
                await self.client.send_message(
                    ...
                    "• Пришлите файл `Jarvis_ДАТА.db` — восстановлю\n"
                    "• Напишите `Джарвис, пустая база` — запущусь без данных"
                )
        except Exception:
            pass
    ...
    async def _handle(self, event):
        ...
        if is_owner and is_pm and any(t in q_own for t in ["восстанови бэкап", "восстанови последний", "восстанови последний бэкап"]):
            ...
            try:
                ok = True  # восстановление через Telegram
                if ok:
        ...
        if is_owner and is_pm and any(t in text.lower() for t in _DB_TRIGGERS):
            ...
            try:
                ...
                try:
                    await self.client.send_file(
                        event.chat_id, _db_p,
                        caption=f"💾 Jarvis_{_label}.db"
                    )
                    await self.client.send_file(
                        event.chat_id, _json_p,
                        caption=f"📄 Jarvis_{_label}.json"
                    )
                    await event.reply(
                        f"✅ Бэкап готов, Сэр!\n\n"
                        f"📊 Статистика:\n"
                    ...
                    await event.reply(f"❌ Ошибка отправки: {_sf_e}")
                finally:
                    try: os.unlink(_json_p)
                    except: pass
                    try: os.unlink(_db_p)
