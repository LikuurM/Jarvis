...
class BackgroundLearner:
    ...
    async def _run_session(self):
        ...
        for row in rows:
            ...
            elif role == "jarvis" and len(text) > 80:
    ...
    async def _handle(self, event):
        ...
        if any(t in text.lower() for t in _STATS_TRIGGERS):
            ...
            try:
                ...
                total_msgs   = st.get("current", 0)
                unique_users = st.get("unique_users", 0)
                deleted_cnt  = st.get("deleted", 0)
                current      = st.get("current", 0)
                previous     = st.get("previous", 0)

                # Считаем процент изменения — ограничиваем ±999%
                if previous > 0:
                ...
                if total_msgs == 0:
                    await event.reply(
                        ...
                        "Сэр, сообщений за этот период не найдено.\n"
                        "Убедитесь что бот добавлен в группу и GROUP_ID настроен."
                    )
                    return
