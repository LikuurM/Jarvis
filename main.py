...
class JarvisAgent:
    ...
    async def process(self, text: str, sender_id: int = 0, username: str = "", chat_id: int = 0) -> str:
        ...
        if any(t in q_lower for t in _TRANSLATE_TRIGGERS):
            # Прямой вызов LLM с жёстким системным промтом — без личности Джарвиса
            translate_messages = [
                {
                    "role": "system",
                    "content": (
                        "Ты переводчик. Твоя единственная задача — перевести текст."
                        " Возвращай ТОЛЬКО перевод. Никаких вступлений, приветствий,"
                        " комментариев, объяснений. Только сам перевод."
                    )
                },
