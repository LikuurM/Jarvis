...
class UniversalLLMConnector:
    """
    Приоритет моделей:
      1. Gemini 2.0 Flash (основной) — быстрый, умный, 1500 запросов/день
      2. Groq llama-4-scout (fallback) — если Gemini исчерпал лимит
      3. Groq llama-3.3-70b (fallback для сложных) — резервный
    ...
    def __init__(self):
        ...
        self._gemini_key = _os_g.getenv("GEMINI_API_KEY",
                           config.GEMINI_API_KEY if hasattr(config, "GEMINI_API_KEY") else "")
        self._gemini_ok  = True  # False когда лимит исчерпан
        if not self._groq_key:
            logger.warning("⚠️ GROQ_API_KEY не задан в .env")
        if self._gemini_key:
            logger.info(f"✅ Gemini 2.0 Flash активирован как основная модель")
    ...
    def current_display(self) -> str:
        if self._gemini_key and self._gemini_ok:
            return f"Gemini 2.0 Flash (основной) + Groq (резерв)"
        return f"Groq / {self.MODEL_FAST} + {self.MODEL_SMART}"

    @property
    def current_spec(self) -> dict:
        return {"type": "gemini" if (self._gemini_key and self._gemini_ok) else "groq",
                "model": self.GEMINI_MODEL if (self._gemini_key and self._gemini_ok) else self.MODEL_FAST}

    def list_models(self) -> str:
        gemini_status = "✅ активен" if (self._gemini_key and self._gemini_ok) else ("⚠️ лимит исчерпан" if self._gemini_key else "❌ ключ не задан")
        groq_status   = "✅ ключ задан" if self._groq_key else "❌ не задан"
        return (
            "**🤖 Модели Джарвиса:**\n\n"
            f"  1️⃣ Gemini 2.0 Flash: {gemini_status}\n"
            f"     1500 запросов/день, сбрасывается в полночь UTC\n\n"
            f"  2️⃣ Groq llama-4-scout: {groq_status} (резерв)\n"
            f"  3️⃣ Groq llama-3.3-70b: {groq_status} (резерв сложных задач)\n\n"
    ...
    def add_custom(self, *a, **kw) -> str:
        return "Сэр, используется Gemini + Groq с автовыбором."

    async def _call_gemini(self, messages: list[dict], max_tokens: int) -> str:
        """Вызов Gemini 2.0 Flash через REST API."""
        import json as _j
        ...
        if r.status_code == 200:
            ...
            logger.warning("⚠️ Gemini лимит исчерпан — переключаюсь на Groq")
            raise Exception("gemini_limit")
        else:
            raise Exception(f"Gemini HTTP {r.status_code}: {r.text[:200]}")

    async def complete(self, messages: list[dict], max_tokens: int = 1400) -> str:
        """Основной вызов: сначала Gemini, при лимите — Groq."""
        ...
        logger.debug(f"🤖 Groq fallback: {chosen_model}")
        ...
        def _call(model: str):
            ...
            return client.chat.completions.create(
                ...
                top_p=1, reasoning_effort="medium", stream=False,
...
class JarvisAgent:
    ...
    async def process(self, text: str, sender_id: int = 0, username: str = "", chat_id: int = 0) -> str:
        ...
        if is_cmp:
            ...
            # Собираем URLs из deep_research
            for m in re.finditer(r"URL: (https?://\S+)", web):
                u = m.group(1)
                if u not in urls_collected:
            ...
            # Собираем URLs из результатов поиска
            skip_domains = (
                "youtube.com","youtu.be","vk.com","instagram.com",
                ...
                "amazon.com","aliexpress.com","wildberries.ru","ozon.ru",
                "ad.","/ads/","banner",
            )
            ...
            # Читаем страницы для углублённого контекста
            urls_fetched = 0
            ...
            if not web_parts:
                logger.warning("Поиск не дал результатов — отвечаем из LLM")
                answer = await self.call_llm(query=query, sender_id=sender_id)
                self.chat_history.save_message(sender_id, "jarvis", answer)
                return answer

            # ── Проверка релевантности результатов ───────────
            # Берём ключевые слова запроса и ищем их в результатах
            _query_words = set(re.findall(r'\b\w{4,}\b', query.lower()))
            _search_text = " ".join(web_parts).lower()
            _matches = sum(1 for w in _query_words if w in _search_text)
            _relevance = _matches / max(len(_query_words), 1)

            if _relevance < 0.15:
                # Результаты нерелевантны — отвечаем из LLM напрямую
                logger.warning(f"⚠️ Результаты поиска нерелевантны ({_relevance:.0%}) — LLM напрямую")
                answer = await self.call_llm(query=query, sender_id=sender_id)
                self.chat_history.save_message(sender_id, "jarvis", answer)
                return answer
        ...
        # AI форматирует ответ на основе найденных данных
