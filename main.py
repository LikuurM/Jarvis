...
class UniversalLLMConnector:
    """
    Приоритет моделей:
      1. GigaChat (основной) — Сбер, русский язык, бесплатно
      2. Groq gpt-oss-120b (резерв) — если GigaChat недоступен
    Переключение автоматическое и тихое.
    """
    ...
    GIGACHAT_AUTH_KEY = "MDE5Y2Y0MmMtMzQzMC03ZTdmLTliMGItN2I1MjI1NTRhZjQ2OmFmNjE4MDg2LTE2ZmEtNDllZS1hN2U5LTVjYzFlZTg5NDg4ZA=="
    ...
    def __init__(self):
        ...
        self._gc_auth_key = _os_g.getenv("GIGACHAT_AUTH_KEY", self.GIGACHAT_AUTH_KEY)
        self._gc_token    = ""
        self._gc_token_exp = 0.0
        self._gc_ok       = True
        if not self._groq_key:
            logger.warning("⚠️ GROQ_API_KEY не задан в .env")
        logger.info("✅ GigaChat активирован как основная модель")

    async def _gigachat_get_token(self) -> str:
        """Получает access token GigaChat. Кэширует на 29 минут."""
        import time as _t, uuid as _uuid
        if self._gc_token and _t.time() < self._gc_token_exp:
            ...
            async with httpx.AsyncClient(timeout=10, verify=False) as cl:
                r = await cl.post(
                    ...
                    headers={
                        ...
                        "Authorization": f"Basic {self._gc_auth_key}",
                    },
                    data={"scope": self.GIGACHAT_SCOPE},
                )
            if r.status_code == 200:
                data = r.json()
                self._gc_token = data.get("access_token", "")
                # expires_at в миллисекундах
                exp = data.get("expires_at", 0)
                self._gc_token_exp = exp / 1000 - 60 if exp > 1e12 else _t.time() + 1680
                logger.debug("✅ GigaChat токен получен")
                return self._gc_token
            else:
                logger.error(f"❌ GigaChat auth: {r.status_code} {r.text[:200]}")
                return ""
        except Exception as e:
            logger.error(f"❌ GigaChat auth ошибка: {e}")
            return ""

    async def _call_gigachat(self, messages: list[dict], max_tokens: int) -> str:
        """Вызов GigaChat API."""
        token = await self._gigachat_get_token()
        if not token:
            raise Exception("gigachat_no_token")

        # Конвертируем messages — GigaChat не принимает system в середине
        clean = []
        sys_content = ""
        for m in messages:
        ...
        # Добавляем system как первое user сообщение если нет истории
        if sys_content and clean:
            clean[0]["content"] = f"{sys_content}\n\n{clean[0]['content']}"
        ...
        if r.status_code == 200:
            ...
            self._gc_token = ""
            self._gc_token_exp = 0
            raise Exception("gigachat_token_expired")
        elif r.status_code == 429:
            self._gc_ok = False
            logger.warning("⚠️ GigaChat лимит — переключаюсь на Groq")
            raise Exception("gigachat_limit")
        else:
            raise Exception(f"GigaChat HTTP {r.status_code}: {r.text[:200]}")

    def _pick_groq_model(self, messages: list[dict]) -> str:
        last_user = ""
        for m in reversed(messages):
            if m.get("role") == "user":
                content = m.get("content", "")
                last_user = " ".join(p.get("text","") for p in content if isinstance(p,dict)) if isinstance(content, list) else str(content)
                break
        q = last_user.lower()
        if len(last_user) > 200 or any(t in q for t in self._SMART_TRIGGERS):
    ...
    def current_display(self) -> str:
        if self._gc_ok:
            return f"GigaChat (основной) + Groq (резерв)"
        return f"Groq {self.MODEL_FAST}"

    @property
    def current_spec(self) -> dict:
        return {"type": "gigachat" if self._gc_ok else "groq",
                "model": self.GIGACHAT_MODEL if self._gc_ok else self.MODEL_FAST}

    def list_models(self) -> str:
        gc_status   = "✅ активен" if self._gc_ok else "⚠️ недоступен"
        groq_status = "✅ ключ задан" if self._groq_key else "❌ не задан"
        return (
            "**🤖 Модели Джарвиса:**\n\n"
            f"  1️⃣ GigaChat: {gc_status} (основной, Сбер)\n"
            f"  2️⃣ Groq {self.MODEL_FAST}: {groq_status} (резерв)\n\n"
    ...
    def add_custom(self, *a, **kw) -> str:
        return "Сэр, используется GigaChat + Groq с автовыбором."

    async def complete(self, messages: list[dict], max_tokens: int = 1400) -> str:
        """Основной вызов: GigaChat → Groq при недоступности."""
        import concurrent.futures as _cf, asyncio as _aio

        # ── 1. GigaChat ───────────────────────────────────────
        if self._gc_ok:
            try:
                result = await self._call_gigachat(messages, max_tokens)
                if result:
                    return result
            except Exception as e:
                err_s = str(e)
                if "gigachat_token_expired" in err_s:
                    # Один повтор с новым токеном
                    try:
                        result = await self._call_gigachat(messages, max_tokens)
                        if result:
                            return result
                        ...
                        pass
                elif "gigachat_limit" not in err_s and "gigachat_no_token" not in err_s:
        ...
        chosen_model = self._pick_groq_model(messages)
        logger.info(f"🔄 Переключение на Groq: {chosen_model}")
        ...
        def _call(model: str):
            ...
            for m in messages:
                if isinstance(m.get("content"), list):
                    txt = " ".join(p.get("text","") for p in m["content"] if isinstance(p,dict) and p.get("type")=="text")
                    clean.append({"role": m["role"], "content": txt})
                else:
                    clean.append(m)
            return client.chat.completions.create(
                model=model, messages=clean,
                temperature=1, max_completion_tokens=min(max_tokens, 8192),
...
class JarvisAgent:
    ...
    async def web_search(self, query: str, n: int = 7) -> list[str]:
        """
        Поиск: DuckDuckGo для ссылок + Jina AI Reader для чтения страниц.
        Jina (r.jina.ai) бесплатно читает любой сайт и возвращает чистый текст.
        """
        results = []

        # ── 1. Получаем ссылки через DuckDuckGo ──────────────
        urls = []
        try:
            with DDGS() as ddg:
                for r in ddg.text(query, max_results=n, region="ru-ru"):
                    ...
                    url     = r.get("href", "")
                    if url:
                        urls.append((title, snippet, url))
            logger.info(f"🔍 DDG: {len(urls)} ссылок по запросу")
        except Exception as e:
        ...
        # ── 2. Читаем страницы через Jina AI Reader ───────────
        # r.jina.ai/{url} — бесплатно, возвращает чистый текст страницы
        skip_domains = (
        ...
        pages_read = 0
        for title, snippet, url in urls:
            # Сначала добавляем сниппет как быстрый результат
            results.append(f"[{title}]\n{snippet}\nURL: {url}")

            # Пробуем прочитать страницу через Jina
            if pages_read >= 4:
                continue
            ...
            try:
                ...
                async with httpx.AsyncClient(
                    ...
                    resp = await cl.get(jina_url)
                    if resp.status_code == 200:
                        text = resp.text.strip()
                        if len(text) > 200:
                            # Заменяем сниппет полным текстом
                            results[-1] = f"[{title}]\n{text[:2500]}\nURL: {url}"
                            pages_read += 1
                            logger.debug(f"📄 Jina прочитал: {url[:60]}")
            except Exception:
                pass  # Jina не смог — остаётся сниппет

        logger.info(f"🔍 Итого: {len(results)} результатов, {pages_read} страниц прочитано через Jina")
        return results
    ...
    def _build_ml_context(self, sender_id: int) -> str:
        ...
        try:
            ...
            try:
                _pat_rows = _jarvis_db._q(
                    "SELECT topic, COUNT(*) as cnt FROM ml_patterns WHERE sender_id=? "                    "GROUP BY topic ORDER BY cnt DESC LIMIT 3",
                    (sender_id,), fetch="all") or []
                if _pat_rows:
                    interests = ", ".join(r.get("topic","") for r in _pat_rows if r.get("topic"))
                    if interests:
                        parts.append(f"Интересы пользователя: {interests}.")
            except Exception:
                pass
    ...
    async def call_llm(self, query: str, context: str = "", rag_context: str = "",
        ...
        if context:
            user_content = (
                f"ДАННЫЕ ИЗ ИНТЕРНЕТА (используй их для ответа, они актуальные):\n"
                f"{context[:3000]}\n\n"
                f"ВОПРОС: {query}\n\n"
                f"Ответь ТОЛЬКО на основе данных выше. Если там есть конкретные цифры/факты — используй их. "
                f"Не ссылайся на свои старые знания."
            )
    ...
    async def process(self, text: str, sender_id: int = 0, username: str = "", chat_id: int = 0) -> str:
        ...
        # ── ВСЕ остальные запросы — через поиск ──────────────
        ...
        if is_cmp:
            ...
            # Убираем год из запроса — мешает поиску по аниме/культуре
            _now = datetime.now()
            _search_query = query
            search_results = await self.web_search(_search_query, 7)

            if not search_results:
                # Поиск ничего не вернул — LLM напрямую
                logger.warning("Поиск не дал результатов — LLM напрямую")
                answer = await self.call_llm(query=query, sender_id=sender_id)
                self.chat_history.save_message(sender_id, "jarvis", answer)
                return answer
            ...
            # Собираем URLs для чтения страниц
            skip_domains = (
                "youtube.com","youtu.be","vk.com","instagram.com",
            ...
            for r in search_results:
                ...
                if m:
                    u = m.group(1)
                    if not any(d in u for d in skip_domains) and u not in urls_collected:
                        urls_collected.append(u)

            # Читаем 3 страницы через Jina для углублённого контекста
            urls_fetched = 0
            for url in urls_collected[:3]:
                page_text = await self.fetch_page(url, max_chars=3000)
                if page_text and len(page_text) > 200:
                    web_parts.append(f"[Страница: {url}]\n{page_text}")
                    urls_fetched += 1
        ...
        # Если ответ говорит что нет данных — пробуем LLM напрямую
        _no_data_phrases = ["нет информации", "нет данных", "не содержит", "не найдено", "данных нет"]
        if any(p in answer.lower() for p in _no_data_phrases) and len(answer) < 300:
            logger.info("🔄 Поиск не дал ответа — пробуем LLM напрямую")
            answer = await self.call_llm(query=query, sender_id=sender_id)
...
class BackgroundLearner:
    ...
    async def _run_session(self):
        ...
        # Сохраняем прогресс — не перечитываем старое при перезапуске
        await loop.run_in_executor(None, self._save_last_ids)
        if rows:
            logger.info(f'🧠 ML сессия: {len(rows)} сообщ., {patterns_new} паттернов, {knowledge_new} знаний')
    ...
    def _fetch_new(self):
        ...
        # Групповые сообщения — отдельная таблица
    ...
    def _load_last_ids(self):
        """Загружает последние обработанные ID из БД (сохраняется между перезапусками)."""
        try:
            row = self._db._q(
    ...
    def _save_last_ids(self):
        """Сохраняет прогресс чтобы не перечитывать старые сообщения при перезапуске."""
        try:
            self._db._q(
    ...
    async def _reminder_loop(self):
        ...
        while True:
            try:
                ...
                if _now.weekday() == 6 and _now.hour == 22 and _now.minute == 0:
    ...
    async def _handle(self, event):
        ...
        if any(p in q_own for p in ["покажи команды","список команд","команды джарвиса","помощь","help","что умеешь","команды","все команды"]):
            cmd_list = (
                "🤖 **Джарвис — Все команды**\n\n"
                "**💬 Основное:**\n"
                "`Джарвис, [вопрос]` — спросить ИИ (Gemini / Groq)\n"
                "`Джарвис, стоп` — пауза\n"
        ...
        if any(t in text.lower() for t in _STATS_TRIGGERS):
            ...
            if not target_id:
                ...
                return
            try:
                _days = 30 if "месяц" in text.lower() else 7
                st = _jarvis_db.get_group_stats(target_id, _days)
                if not st:
                    await event.reply("Сэр, данных по группе пока нет.")
                    return
                arrow = "📈" if st.get("change", 0) >= 0 else "📉"
                sign  = "+" if st.get("change", 0) >= 0 else ""
                top_lines = "\n".join(
                ...
                msg = (
                    f"📊 **Статистика группы (за {_days} дней):**\n\n"
                    f"💬 Сообщений: {st.get('total',0)}\n"
                    f"👤 Участников: {st.get('users_count',0)}\n"
                    f"{arrow} Активность: {sign}{st.get('change',0)}% vs предыдущий период\n"
