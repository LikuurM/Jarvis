...
class JarvisAgent:
    ...
    async def youtube_search(self, query: str, n: int = 5) -> str:
        """Поиск видео на YouTube через Data API v3. Возвращает рабочие ссылки."""
        ...
        try:
            ...
            lines = [f"🎬 YouTube — результаты по запросу «{query}»:\n"]
    ...
    async def process(self, text: str, sender_id: int = 0, username: str = "", chat_id: int = 0) -> str:
        ...
        if any(t in q_lower for t in _WIKI_EXPLICIT):
            ...
            return await self.wikipedia_search(query, full=is_full)
        ...
        if any(t in q_lower for t in _YT_TRIGGERS):
            ...
            if yt_query:
                return await self.youtube_search(yt_query)
...
class JarvisTelegram:
    ...
    async def _reminder_loop(self):
        """Проверяет напоминания каждые 10 сек + еженедельная статистика."""
        await asyncio.sleep(3)
        _last_weekly = None  # чтобы не слать дважды в одну минуту
        while True:
            try:
