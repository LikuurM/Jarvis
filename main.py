        ...
        if config.GOOGLE_SERVICE_ACCOUNT:
            try:
                ...
                async with httpx.AsyncClient(timeout=10, verify=False) as cl:
                    r = await cl.get(
            "https://www.googleapis.com/v1/disk",
            headers={"Authorization": f"OAuth {config.GOOGLE_SERVICE_ACCOUNT}"},
                    )
                ok = r.status_code == 200
                self.update("Google Drive", ok, "" if ok else f"HTTP {r.status_code}")
                results["yandex"] = ok
            except Exception as e:
                self.update("Google Drive", False, str(e)[:60])
...
class UniversalLLMConnector:
    """
    Два режима Groq:
      FAST  — llama-4-scout-17b  (быстрый, повседневные вопросы)
      SMART — llama-3.3-70b      (сложные задачи: код, анализ, споры, математика)
    Выбор происходит автоматически по запросу.
    """

    MODEL_FAST  = "meta-llama/llama-4-scout-17b-16e-instruct"
    MODEL_SMART = "llama-3.3-70b-versatile"

    # Триггеры сложных задач → переключаемся на 70b
    _SMART_TRIGGERS = [
    ...
    def _pick_model(self, messages: list[dict]) -> str:
        """Выбирает модель по содержимому запроса."""
        ...
        for m in reversed(messages):
            if m.get("role") == "user":
                ...
                if isinstance(content, list):
                    last_user = " ".join(p.get("text","") for p in content if isinstance(p,dict))
                else:
                    last_user = str(content)
                break
        q = last_user.lower()
        # Длинный запрос (>200 символов) → 70b
        if len(last_user) > 200:
            return self.MODEL_SMART
        # Ключевые слова → 70b
        if any(t in q for t in self._SMART_TRIGGERS):
            return self.MODEL_SMART
        return self.MODEL_FAST

    @property
    def current_display(self) -> str:
        return f"Groq / {self.MODEL_FAST} + {self.MODEL_SMART}"

    @property
    def current_spec(self) -> dict:
        return {"type": "groq", "model": self.MODEL_FAST}
    ...
    def list_models(self) -> str:
        ...
        return (
            "**🚀 Groq — два режима:**\n"
            f"  ⚡ Быстрый: `{self.MODEL_FAST}`\n"
            f"  🧠 Умный:  `{self.MODEL_SMART}`\n"
            f"  Статус: {status}\n\n"
            "Выбор модели — автоматически по сложности запроса."
        )
    ...
    def switch(self, query: str) -> str | None:
        ...
        if any(p in q for p in ["текущая модель", "какая модель", "что используешь", "смени модель", "список моделей"]):
            return (
                "Сэр, использую два режима через Groq:\n\n"
                + self.list_models()
            )
        return None

    def add_custom(self, *a, **kw) -> str:
        return "Сэр, используется только Groq с автовыбором модели."

    async def complete(self, messages: list[dict], max_tokens: int = 1400) -> str:
        """Вызов Groq с автовыбором модели. ThreadPoolExecutor чтобы не блокировать asyncio."""
        ...
        logger.debug(f"🤖 Модель: {chosen_model}")
        ...
        try:
            ...
            # Если scout упал — fallback на 70b
            if chosen_model == self.MODEL_FAST and ("503" in err or "unavailable" in err or "model" in err.lower()):
                logger.warning("⚠️ Scout недоступен — fallback на 70b")
                try:
                    with _cf.ThreadPoolExecutor(max_workers=2) as ex:
                        return await loop.run_in_executor(ex, _call, self.MODEL_SMART)
                except Exception as e2:
                    err = str(e2)
            if "401" in err or "403" in err or "Forbidden" in err or "invalid_api_key" in err:
                key_hint = self._key[:8] + "..." if self._key else "(пусто)"
                logger.error(f"Groq 401/403: ключ={key_hint}")
                return "Сэр, GROQ_API_KEY неверный или не активирован. Проверьте .env на сервере."
        ...
        if config.GOOGLE_SERVICE_ACCOUNT:
            try:
                ...
                if _ydr.status_code == 200:
                    ...
                    results.append(("Google Drive", True, f"✅ {_used}/{_total} GB"))
                else:
                    results.append(("Google Drive", False, f"токен устарел (HTTP {_ydr.status_code}) — обновите в .env"))
            except Exception as e:
                results.append(("Google Drive", False, str(e)[:60]))
        else:
            results.append(("Google Drive", False, "Google Service Account не настроен"))
...
# ═══════════════════════════════════════════════════════════════════
#  GOOGLE DRIVE BACKUP — Service Account (без OAuth, без токенов)
#  Ключ: jarvis@nimble-factor-454016-q3.iam.gserviceaccount.com
# ═══════════════════════════════════════════════════════════════════

_gdrive_access_token: str = ""
_gdrive_token_expires: float = 0.0


def _gdrive_make_jwt() -> str:
    """Создаёт JWT для Service Account. Использует google-auth или rsa."""
    import json as _j, base64 as _b, time as _t, hashlib as _h, struct as _s
    sa = config.GOOGLE_SERVICE_ACCOUNT
    if not sa:
        return ""
    try:
        # Вариант 1: google-auth (предпочтительный)
        ...
        return creds.token  # уже готовый access token, не JWT
    except ImportError:
        pass
    try:
        # Вариант 2: cryptography
        from cryptography.hazmat.primitives import hashes, serialization
        from cryptography.hazmat.primitives.asymmetric import padding
        header  = _b.urlsafe_b64encode(_j.dumps({"alg": "RS256", "typ": "JWT"}).encode()).rstrip(b"=")
        now     = int(_t.time())
        claims  = {"iss": sa["client_email"],
                   "scope": "https://www.googleapis.com/auth/drive.file",
                   "aud": "https://oauth2.googleapis.com/token",
                   "iat": now, "exp": now + 3600}
        payload = _b.urlsafe_b64encode(_j.dumps(claims).encode()).rstrip(b"=")
        msg     = header + b"." + payload
        key = serialization.load_pem_private_key(sa["private_key"].encode(), password=None)
        sig = _b.urlsafe_b64encode(key.sign(msg, padding.PKCS1v15(), hashes.SHA256())).rstrip(b"=")
        return (msg + b"." + sig).decode()
    except ImportError:
        pass
    try:
        # Вариант 3: rsa (лёгкая библиотека)
        import rsa as _rsa
        header  = _b.urlsafe_b64encode(_j.dumps({"alg": "RS256", "typ": "JWT"}).encode()).rstrip(b"=")
        now     = int(_t.time())
        claims  = {"iss": sa["client_email"],
        ...
        payload = _b.urlsafe_b64encode(_j.dumps(claims).encode()).rstrip(b"=")
        msg     = header + b"." + payload
        priv_key = _rsa.PrivateKey.load_pkcs1_openssl_pem(sa["private_key"].encode())
        sig = _b.urlsafe_b64encode(_rsa.sign(msg, priv_key, "SHA-256")).rstrip(b"=")
        return (msg + b"." + sig).decode()
    except ImportError:
        logger.error("❌ Google Drive: установите google-auth или rsa: pip install google-auth")
        return ""


async def _gdrive_refresh_token() -> str:
    """Получает access_token через Service Account. Кэширует на 55 минут."""
    global _gdrive_access_token, _gdrive_token_expires
    import time as _t, concurrent.futures as _cf, asyncio as _aio
    if _gdrive_access_token and _t.time() < _gdrive_token_expires:
        return _gdrive_access_token
    if not config.GOOGLE_SERVICE_ACCOUNT:
        return ""
    try:
        # _gdrive_make_jwt возвращает либо JWT либо уже готовый access_token (google-auth)
        loop = _aio.get_event_loop()
        with _cf.ThreadPoolExecutor(max_workers=1) as ex:
            token_or_jwt = await loop.run_in_executor(ex, _gdrive_make_jwt)
        if not token_or_jwt:
            return ""
        # Если google-auth вернул готовый access_token (начинается с ya29.)
        if token_or_jwt.startswith("ya29."):
            _gdrive_access_token  = token_or_jwt
            _gdrive_token_expires = _t.time() + 3500
            return _gdrive_access_token
        # Иначе это JWT — обмениваем на access_token
        async with httpx.AsyncClient(timeout=15, verify=False) as cl:
            r = await cl.post(
                "https://oauth2.googleapis.com/token",
                data={"grant_type": "urn:ietf:params:oauth:grant-type:jwt-bearer", "assertion": token_or_jwt},
            )
        if r.status_code == 200:
            data = r.json()
            _gdrive_access_token  = data.get("access_token", "")
            _gdrive_token_expires = _t.time() + data.get("expires_in", 3600) - 60
            return _gdrive_access_token
        logger.error(f"❌ Google Drive: ошибка токена ({r.status_code}): {r.text[:200]}")
        return ""
    except Exception as e:
        logger.error(f"❌ Google Drive: JWT ошибка: {e}")
        return ""


async def _gdrive_get_or_create_folder() -> str:
    """Возвращает ID папки Jarvis_Backup. Если GOOGLE_DRIVE_FOLDER_ID задан — использует его."""
    # Если пользователь задал ID напрямую — используем его (папка расшарена вручную)
    if config.GOOGLE_DRIVE_FOLDER_ID:
        return config.GOOGLE_DRIVE_FOLDER_ID
    token  = await _gdrive_refresh_token()
    folder = config.GOOGLE_DRIVE_FOLDER
    if not token:
        return ""
    try:
        async with httpx.AsyncClient(timeout=15, verify=False) as cl:
            ...
            r = await cl.get(
                ...
                headers=h,
                params={"q": f"mimeType='application/vnd.google-apps.folder' and name='{folder}' and trashed=false",
            ...
            cr = await cl.post(
                ...
                json={"name": folder, "mimeType": "application/vnd.google-apps.folder"},
            )
            return cr.json().get("id", "")
    except Exception as e:
        logger.error(f"❌ Google Drive: ошибка папки: {e}")
        return ""


async def _upload_to_gdrive(file_path: str, filename: str) -> bool:
    """Загружает файл на Google Drive в папку Jarvis_Backup."""
    token = await _gdrive_refresh_token()
    if not token:
        return False
    try:
        import json as _j
        folder_id = await _gdrive_get_or_create_folder()
        if not folder_id:
            return False
        async with httpx.AsyncClient(timeout=180, verify=False) as cl:
            ...
            r = await cl.get(
                "https://www.googleapis.com/drive/v3/files",
                headers=h,
                params={"q": f"name='{filename}' and '{folder_id}' in parents and trashed=false",
                        "fields": "files(id)"},
            )
            ...
            if existing:
                ...
                meta     = _j.dumps({"name": filename, "parents": [folder_id]}).encode()
                boundary = "jarvis_boundary_xyz"
                body = (
        ...
        if up.status_code in (200, 201):
            logger.info(f"✅ Google Drive: {filename} загружен в {config.GOOGLE_DRIVE_FOLDER}/")
            return True
        logger.error(f"❌ Google Drive: {filename} не загружен (HTTP {up.status_code}): {up.text[:200]}")
        return False
    except Exception as e:
        logger.error(f"❌ Google Drive: сбой загрузки {filename}: {e}")
        return False


async def _gdrive_cleanup_old_backups(keep: int = 14):
    """Удаляет старые бэкапы, оставляет keep штук."""
    token = await _gdrive_refresh_token()
    if not token:
        return
    try:
        ...
        async with httpx.AsyncClient(timeout=30, verify=False) as cl:
            ...
            r = await cl.get(
                "https://www.googleapis.com/drive/v3/files",
                headers=h,
                params={"q": f"'{folder_id}' in parents and trashed=false and name contains 'Jarvis_'",
                        "fields": "files(id,name)", "orderBy": "name desc", "pageSize": 100},
            )
            files = r.json().get("files", [])
            for old in files[keep:]:
                await cl.delete(f"https://www.googleapis.com/drive/v3/files/{old['id']}", headers=h)
                logger.info(f"🗑 Google Drive: удалён {old['name']}")
    except Exception as e:
        logger.warning(f"⚠️ Google Drive: не удалось удалить старые бэкапы: {e}")


async def _gdrive_list_backups() -> list[dict]:
...
async def _restore_from_gdrive_if_empty():
    """Восстанавливает БД с Google Drive при старте если БД пустая."""
    if not config.GOOGLE_SERVICE_ACCOUNT:
        return
    try:
        stats = _jarvis_db.get_db_stats()
        if stats.get("user_messages", 0) + stats.get("group_messages", 0) > 0:
            return
        logger.info("БД пустая — пробуем восстановить с Google Drive...")
        token = await _gdrive_refresh_token()
        if not token:
            logger.warning("⚠️ Google Drive: токен не получен")
            return
        folder_id = await _gdrive_get_or_create_folder()
        if not folder_id:
            return
        async with httpx.AsyncClient(timeout=30, verify=False) as cl:
            ...
            for f in files:
                ...
                if dm:
                    try:
                        delta = abs((_today - datetime.strptime(dm.group(1), "%Y-%m-%d")).days)
                        if _best_delta is None or delta < _best_delta:
                            _best_delta, _best = delta, f
                    except Exception:
                        pass
            if not _best:
                _best = next((f for f in files if f.get("name","").endswith(".db")), None)
            if not _best:
                logger.info("ℹ️ На Google Drive нет .db файлов")
                return
            import tempfile as _tmp, os as _os
            dl = await cl.get(
                f"https://www.googleapis.com/drive/v3/files/{_best['id']}",
                headers=h, params={"alt": "media"}, timeout=180,
            )
            if dl.status_code != 200:
                return
            tmp = _tmp.mktemp(suffix=".db")
            with open(tmp, "wb") as f2:
                f2.write(dl.content)
            ok = _jarvis_db.restore_from_path(tmp)
            _os.unlink(tmp)
            if ok:
                rt = sum(_jarvis_db.get_db_stats().get(k,0) for k in ["user_messages","group_messages"])
                logger.info(f"✅ БД восстановлена из {_best['name']} ({rt} записей)")
    except Exception as e:
        logger.warning(f"⚠️ Восстановление с Google Drive не выполнено: {e}")
                ...
                if _now.hour == 3 and _now.minute < 1:
                    ...
                    if _last_backup != _backup_key and config.OWNER_ID:
                        ...
                        try:
                            ...
                            if config.GOOGLE_SERVICE_ACCOUNT:
                                _ok_j = await _upload_to_gdrive(_json_p, f"Jarvis_{_label}.json")
                                _ok_d = await _upload_to_gdrive(_db_p,   f"Jarvis_{_label}.db")
                                try:
                                    os.unlink(_json_p)
                                    os.unlink(_db_p)
                                except Exception:
                                ...
                                if _ok_j and _ok_d:
                                    ...
                                    await _gdrive_cleanup_old_backups(keep=14)
                                else:
    ...
    async def _handle(self, event):
        ...
        if any(p in q_own for p in ["покажи команды","список команд","команды джарвиса","помощь","help","что умеешь","команды","все команды"]):
            cmd_list = (
                ...
                "**💾 Бэкап:**\n"
                "`Джарвис, сделай бэкап` — на Google Drive\n"
        ...
        # ── Список бэкапов на Google Driveе ──────────────────
        if is_owner and is_pm and any(t in q_own for t in ["список бэкапов", "мои бэкапы", "бэкапы яндекс"]):
            _bk_list = await _gdrive_list_backups()
            if not _bk_list:
                await event.reply("Сэр, бэкапов на Google Drive не найдено.")
            else:
                _lines = ["📦 Бэкапы на Google Drive:"]
        ...
        if is_owner and is_pm and any(t in q_own for t in ["восстанови бэкап", "восстанови последний", "восстанови последний бэкап"]):
            if not config.GOOGLE_SERVICE_ACCOUNT:
                await event.reply("Сэр, Google Service Account не настроен в .env")
                return
            await event.reply("⏳ Ищу последний бэкап на Google Drive...")
            try:
                ok = await _restore_from_gdrive_if_empty() or True
                if ok:
                    await event.reply("✅ База восстановлена из последнего бэкапа, Сэр!")
                else:
                    await event.reply("❌ Не удалось восстановить с Google Drive.")
        ...
        if is_owner and is_pm and "восстанови базу за" in q_own:
            ...
            if m_rst:
                ...
                _bks = await _gdrive_list_backups()
                ...
                if not _found_bk:
                    ...
                    try:
                        token_r = await _gdrive_refresh_token()
                        async with httpx.AsyncClient(timeout=180, verify=False) as cl_r:
                            dl = await cl_r.get(
                                f"https://www.googleapis.com/drive/v3/files/{_found_bk['id']}",
                            ...
                            if dl.status_code == 200:
                                ...
                                await event.reply(f"❌ Google Drive вернул {dl.status_code}")
        ...
        if is_owner and is_pm and any(t in text.lower() for t in _DB_TRIGGERS):
            ...
            try:
                ...
                if config.GOOGLE_SERVICE_ACCOUNT:
                    ...
                    if _ok_j and _ok_d:
                        await event.reply(
                            f"✅ Бэкап загружен на Google Drive, Сэр!\n\n"
                            f"📁 {config.GOOGLE_DRIVE_FOLDER}/\n"
                        ...
                        # Google Drive не работает — отправляем файлы напрямую
                        await event.reply("⚠️ Google Drive недоступен, отправляю файлы напрямую, Сэр...")
                        try:
                            await self.client.send_file(event.chat_id, _db_p, caption=f"💾 Jarvis_{_label}.db")
                            await self.client.send_file(event.chat_id, _json_p, caption=f"📄 Jarvis_{_label}.json")
                    ...
                    await event.reply("⚠️ Google Service Account не настроен. Отправляю файлы в Telegram, Сэр...")
                    try:
        ...
        # ── Проверка бэкапов на Google Driveе ─────────────────
        ...
        if is_owner and is_pm and any(t in q_own for t in _CHK_BACKUP):
            ...
            try:
                ...
                _h = {"Authorization": f"OAuth {config.GOOGLE_SERVICE_ACCOUNT}"}
                async with _hx_bk.AsyncClient(timeout=15, verify=False) as _cl:
                    _r = await _cl.get(
                        "https://www.googleapis.com/v1/disk/resources",
                        params={"path": config.GOOGLE_DRIVE_FOLDER, "limit": 50, "sort": "-modified"},
                        headers=_h)
                if _r.status_code != 200:
                    await event.reply(f"Сэр, Google Drive недоступен (HTTP {_r.status_code}). Обновите токен.")
                    return
                ...
                if not _db_files:
                    await event.reply("Сэр, бэкапов (.db) на Google Driveе не найдено.")
                    return
                _latest = _db_files[0]
                ...
                _ml = _jarvis_db.ml_stats()
                _lines = [
                    "☁️ **Google Drive — бэкапы Джарвиса:**",
                    "",
                    f"📦 Последний: `{_name}`",
                    f"📅 Изменён: {_mod}",
                    f"💾 Размер: {_size} KB",
...
async def main():
    ...
    # ── Восстановление с Google Driveа если БД пустая ─────
    await _restore_from_gdrive_if_empty()
