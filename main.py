    ...
    async def complete(self, messages: list[dict], max_tokens: int = 1400) -> str:
        ...
        def _call(model: str):
            ...
            return client.chat.completions.create(
                ...
                temperature=0.7, max_completion_tokens=min(max_tokens, 32768),
...
class JarvisAgent:
    ...
    # Список публичных SearXNG серверов — ротация при недоступности
    _SEARX_INSTANCES = [
        "https://searx.be",
        "https://search.sapti.me",
        "https://searx.tiekoetter.com",
        "https://searx.ollin.one",
    ...
    async def web_search(self, query: str, n: int = 5) -> list[str]:
        """
        Поиск в интернете.
        1. SearXNG (ротация публичных серверов) — основной, актуальные данные
        2. DuckDuckGo — резерв если SearXNG недоступен
        3. Tavily — финальный резерв если есть ключ
        """
        results = []

        # ── 1. SearXNG — основной поиск ──────────────────────────────
        _tried = 0
        while _tried < len(self._SEARX_INSTANCES) and not results:
            _idx  = self.__class__._searx_index % len(self._SEARX_INSTANCES)
            _base = self._SEARX_INSTANCES[_idx]
            _tried += 1
            try:
                async with httpx.AsyncClient(
                    ...
                    if resp.status_code == 200:
                        ...
                        if results:
                            logger.info(f"🔍 SearXNG [{_base}]: {len(results)} результатов")
                    else:
                        # Сервер недоступен — переходим к следующему
                        self.__class__._searx_index += 1
            except Exception as _se:
                logger.debug(f"SearXNG [{_base}] ошибка: {_se}")
                self.__class__._searx_index += 1
            ...
            with DDGS() as ddg:
                for r in ddg.text(query, max_results=n):
                    ...
                    if len(results) >= n:
                        break
            if results:
                logger.info(f"🔍 DuckDuckGo (резерв): {len(results)} результатов")
                return results
        except Exception as e:
            logger.debug(f"DDG search error: {e}")

        # ── 3. Tavily — финальный резерв ─────────────────────────────
        if config.TAVILY_API_KEY:
            try:
                ...
                if results:
                    logger.info(f"🔍 Tavily (резерв): {len(results)} результатов")
            except Exception as e:
...
class BackgroundLearner:
    ...
    def _fetch_new(self):
        ...
        try:
            raw = self._db._q(
                "SELECT id, sender_id, text, date FROM group_messages "
