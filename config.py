...
# BotHost хранит данные в /app/data/ — эта папка переживает перезапуски.
# Детектируем по наличию /app/data или переменной окружения DATA_DIR.
_DATA_DIR_ENV = os.getenv("DATA_DIR", "")
if _DATA_DIR_ENV:
    DIR_DATABASE = _Path(_DATA_DIR_ENV)
elif _sys.platform != "win32" and _Path("/app/data").exists():
    DIR_DATABASE = _Path("/app/data")
elif _sys.platform != "win32" and _Path("/app").exists():
    DIR_DATABASE = _Path("/app/data")
else:
...
DIR_SESSIONS  = DIR_DATABASE / "sessions"
DIR_LOGS      = DIR_DATABASE / "logs"
