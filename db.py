...
class JarvisDB:
    ...
    def get_group_stats(self, chat_id: int, days: int = 7) -> dict:
        ...
        try:
            conn = self._connect()
            period_start = datetime.now(timezone(timedelta(hours=3))).replace(tzinfo=None)
            period_start = (period_start - timedelta(days=days)).strftime("%Y-%m-%d %H:%M:%S")
            prev_start   = (datetime.now(timezone(timedelta(hours=3))).replace(tzinfo=None)
            ...
            top_users = [dict(r) for r in conn.execute("""
                ...
                GROUP BY sender_id, sender ORDER BY cnt DESC LIMIT 5
            ...
            change = round((current - prev) / prev * 100) if prev else 0
